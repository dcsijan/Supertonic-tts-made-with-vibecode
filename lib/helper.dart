...
typedef TtsProgressCallback = void Function(double progress, String message);
...
class TextToSpeech {
  ...
  Future<Map<String, dynamic>> call(
    ...
    TtsProgressCallback? onProgress, // ✅ NEW
  }) async {
    ...
    if (chunks.isEmpty) {
      onProgress?.call(0.0, 'No text to synthesize');
      return {'wav': <double>[], 'duration': [0.0]};
    }
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      final result = await _infer(
        ...
        speed: speed,
        onProgress: (p, msg) {
          // Map per-chunk progress [0..1] into global [i/chunks .. (i+1)/chunks]
          final base = i / chunks.length;
          final part = 1.0 / chunks.length;
          onProgress?.call(base + p * part, msg);
        },
      );
  ...
  Future<Map<String, dynamic>> _infer(
    ...
    onProgress?.call(0.02, 'Duration predictor...');
    ...
    onProgress?.call(0.06, 'Text encoder...');
    ...
    // Denoising loop (dominant compute)
    for (var step = 0; step < totalStep; step++) {
      final p = totalStep == 0 ? 0.0 : (step / totalStep);
      // Map denoise into ~[0.10..0.90] within this chunk
      onProgress?.call(0.10 + p * 0.80, 'Denoising ${step + 1}/$totalStep...');

      final r = await vectorEstOrt.run({
      ...
      final denoisedRaw = await r.values.first.asList();
      final denoised =
          denoisedRaw is List<double> ? denoisedRaw : _safeCast<double>(denoisedRaw);
    ...
    onProgress?.call(0.92, 'Vocoder...');

    final vocoderResult =
        await vocoderOrt.run({'latent': await _toTensor(noisyLatent, latentShape)});
...
Future<TextToSpeech> loadTextToSpeech(String onnxDir, {bool useGpu = false}) async {
